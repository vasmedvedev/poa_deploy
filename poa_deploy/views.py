...
from poa_deploy.exceptions import AppExists
import json
...
def install_application(request):
	if request.is_ajax():
		...
		if connection is not None and application is not None:
			import_response = utils.import_app_to_poa(application, connection)
			rt_response = utils.create_rt_for_app(import_response['application_id'], application, connection)
			st_id = utils.create_service_template(rt_response['resource_type_id'], application, connection)
		else:
			return None
		response = {'status': 0}
